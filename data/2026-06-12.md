...
**TL;DR**: Semiclassical equations of motion are derived for composite bound states (excitons, trions) in insulators/semiconductors. Neutral composites under a uniform electric field couple not to single-particle Berry curvature but to a distinct quantum-geometric quantity—the difference between inequivalent Berry connections (generalized quantum geometric dipole). Charged composites (trions) acquire an additional Berry-curvature term, but there exists an infinite family of inequivalent composite Berry curvatures; the physically correct one depends on the chosen spatial centre of the composite. Applied to magic-angle twisted bilayer graphene, trions exhibit transverse drift and oscillating internal dipole dynamics due to the interplay of Berry curvature and the quantum geometric dipole.  
**Method**: Analytical derivation of semiclassical equations of motion for general composite bound states, identification and classification of inequivalent Berry connections/curvatures for composites, and application of the formalism to trions in magic-angle twisted bilayer graphene to compute geometric contributions to center-of-mass and internal dynamics.  
**Tags**: `quantum_geometry,berry_curvature,exciton,trion,semiclassical_dynamics,twisted_bilayer_graphene,composite_particle_dynamics,topology`
...
**TL;DR**: A short optical pulse drives wave-like, time-dependent dynamics of the quantum metric near graphene’s Dirac points and generates a transient Berry-curvature wave; parts of the metric dynamics are identified with a measurable Fisher-information wave in conduction/valence populations.  
**Method**: Solve the time-dependent Schrödinger equation for optically driven graphene (neglecting correlations and out-of-equilibrium effects) to extract the momentum-time (kx,ky,t) quantum metric and Berry curvature and relate them to Floquet-band features and pump-probe observables.  
**Tags**: `quantum_geometry, topology, Floquet, Berry_curvature, graphene, Fisher_information, pump-probe, time-dependent_Schrödinger`
...
### [3] [Understanding quantum behaviors of an electron in a uniform magnetic field alternatively](https://arxiv.org/abs/2606.13290)
*Jin-Ming Wang, Yuan-Zao Gao, Dai-Lin Cun, Jian Jing*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2606.13290)

**TL;DR**: Identifies a critical radius in Landau-level eigenstates that splits the plane into an inner clockwise and an outer counterclockwise probability-current region; the vanishing total current for negative angular quantum number is an exact cancellation between these regions. By defining a partitioned kinetic angular momentum about that radius, the authors show the electron carries two opposing rotational components (inner counter-rotation and outer co-rotation), leaving a positive net kinetic angular momentum and providing a dynamical picture for Landau-level infinite degeneracy.  
**Method**: Analytical study of the probability-current density of Landau-level eigenstates: locate a critical radius that partitions flow, define a partitioned kinetic angular momentum about that radius, and demonstrate exact cancellation of regional current contributions and the resultant bidirectional rotation structure.  
**Tags**: `quantum_geometry, vortex, Landau_levels, angular_momentum, probability_current`
...
**TL;DR**: Half-shell semiconductor nanowires proximitized by a metallic superconductor can exhibit h/2e Little–Parks supercurrent oscillations because certain higher-energy transverse modes leak out of the interface quantum well and form circumferential eigenstates with mixed electron–hole character, analogous to full-shell systems. These circumferential states are identified as the microscopic origin of the observed flux-periodic coherence.  
**Method**: Microscopic modeling using an effective Hamiltonian obtained from the Green’s function of the proximitized semiconductor; the superconductor is incorporated via a self-energy term and electrostatic band-bending at the metal–semiconductor interface is modeled as a narrow rectangular quantum well. Eigenstate properties are analyzed through the spatial profiles of normal-state transverse modes and comparisons are made between full- and half-shell geometries.  
**Tags**: `Little-Parks, proximitized_nanowire, self-energy_Green_function, interface_quantum_well, mixed_electron-hole_states, flux_oscillations, quantum_devices`
...
**TL;DR**: Charge pumping in a single-stranded helical tight-binding chain is studied for short-range (SRH) and long-range (LRH) hopping using time-periodic end potentials. LRH produces pronounced plateau-like regions in the pumped dc current versus chemical potential in the adiabatic (sparse-level) regime, while SRH yields parameter-sensitive currents without clear plateaus. Increasing drive frequency induces Floquet side-band mixing that destroys plateaus and makes currents oscillatory. The pumped current is nearly sinusoidal in phase and vanishes at zero phase lag; the hopping decay exponent ℓ_c acts as a geometric control knob that can tune magnitude and sign of the current.  
**Method**: Keldysh non-equilibrium Green's function (NEGF) calculations on a tight-binding helical chain including short- and long-range hopping, driven by time-periodic potentials at the ends; analysis of energy-resolved spectral currents and dc pumped currents, with attention to frequency (Floquet side-band) effects and dependence on the hopping decay exponent ℓ_c.  
**Tags**: `quantum_pumping, helical_chains, long-range_hopping, Keldysh_NEGF, Floquet, quantum_geometry, tight-binding, spectral_current`
...
### [6] [Phonon polariton confinement in isotopically pure MOVPE-grown BN triangles](https://arxiv.org/abs/2606.13483)
...
**TL;DR**: MOVPE-grown epitaxial hexagonal boron nitride supports long-lived phonon polaritons and, when patterned as isotopically pure triangular islands, provides efficient nanoscale confinement with mode patterns tunable by incident wavelength—enabling all-epitaxial microscale polariton resonators for nanophotonics and quantum optics.  
**Method**: Material synthesis via metalorganic vapor phase epitaxy (MOVPE) of BN; near-field characterization using scattering-type scanning near-field optical microscopy (s‑SNOM) and nanoscale Fourier-transform infrared spectroscopy (nano‑FTIR); demonstration of confinement with epitaxial isotopically pure triangular islands and measurement of propagation lengths and mode patterns.  
**Tags**: `phonon_polaritons, hexagonal_boron_nitride (hBN), epitaxial_growth, MOVPE, s-SNOM, nano-FTIR, isotopic_purity, polariton_confinement, nanophotonics, quantum_optics, quantum_devices`
...
### [7] [Thermoelectric information engine driven by an autonomous Maxwell demon across quantum-to-classical transitions](https://arxiv.org/abs/2606.13500)
*Maximiliano Bernal Santibañez, Felipe Barra, Jose Mondaca*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2606.13500)

**TL;DR**: Three-terminal autonomous thermoelectric engine formed by a double quantum dot (two terminals) monitored by a third Coulomb-coupled dot acting as an autonomous Maxwell demon. Using a Redfield master equation and two Lindblad limits, the authors identify two quantum-to-classical crossovers: one controlled by interdot tunneling (coherent→classical) and a second induced by a phonon bath (decoherence and incoherent phonon-assisted transport). They map regimes where coherent tunneling enhances the information-flow/demon mechanism and clarify which thermodynamic Lindblad description applies in each regime.  
**Method**: Microscopic open-system modelling with a Redfield quantum master equation for the three-dot system, analysis of steady states and information/transport metrics, and derivation/identification of two corresponding thermodynamically consistent Lindblad approximations; inclusion of a phonon bath to study decoherence and incoherent transport effects.  
**Tags**: `quantum_devices, quantum_transport, quantum_coherence, autonomous_maxwell_demon, Redfield_master_equation, Lindblad, phonon_decoherence, information_flow, thermoelectric_engine, quantum-classical_transition`
...
**TL;DR**: Experimental demonstration of a micron-scale magnon circulator: chiral spin waves launched by nanowire gratings produce three rectilinear, unidirectional spin-wave channels and genuine 3‑port circulation; operation is narrowband but tunable across 2–8 GHz via an external field (≤100 mT) and grating geometry.  
**Method**: Device design using nanowire-grating-mediated chiral excitation of spin waves to form three rectilinear unidirectional channels; fabrication of a micron-sized circulator and characterization by full 3‑port spin‑wave spectroscopy; frequency tuning achieved by varying external magnetic field (up to 100 mT) and grating dimensions to set wavevectors.  
**Tags**: `quantum_devices,magnonics,spin-waves,non-reciprocity,microwave_circulators,integrated_devices`
...
### [9] [Engineering electrically-switchable quantum anomalous Hall states by spin-orbit coupling](https://arxiv.org/abs/2606.13651)
...
**TL;DR**: Proximity coupling of WSe2 to twisted monolayer–bilayer graphene (tMBG) introduces spin–orbit coupling that reshapes magnetization reversal pathways and stabilizes nonvolatile, gate-driven switching of quantum anomalous Hall (QAH) states. Strong magnetic metastability enables gate tuning between QAH and metallic regimes and between |C|=2 and |C|=1 QAH states without resetting the magnetization, offering a route to engineered topological device functionality.  
**Method**: Experimental proximitization of tMBG with WSe2 combined with magnetotransport measurements to probe nonvolatile gate-driven switching; comparison to non-proximitized graphene devices and interpretation attributing changes in switching and metastability to proximity-induced spin–orbit coupling that locks spin and valley and alters competing magnetic states.  
**Tags**: `quantum_anomalous_Hall, moiré_graphene, proximity_spin-orbit_coupling, chiral_edge_states, magnetic_metastability, topological_devices, tMBG, quantum_devices`
...
### [10] [Nonmonotonic temperature dependence of the thermopower of atomic-size gold contacts](https://arxiv.org/abs/2606.12734)
...
**TL;DR**: Measured thermopower of atomic-size Au contacts (MCBJ) from 18–295 K using a local thermometer and laser heating; observed a nonmonotonic temperature dependence with a minimum ≈−2 μV/K at ~150 K that contradicts the linear Landauer prediction. A Landauer-based model with energy-dependent transmission reproduces the qualitative trend but not quantitatively; authors attribute the effect to phonon-related contributions with sign opposite to bulk phonon drag.  
**Method**: Mechanically controllable break junction (MCBJ) thermopower measurements with an on-chip thermometer and focused laser heating to impose a temperature gradient; conductance histograms to confirm quantum transport; theoretical modelling employing an energy-dependent transmission within the Landauer framework to compare with data.  
**Tags**: `thermopower, atomic_contacts, MCBJ, phonon_contributions, Landauer_model, quantum_transport, thermoelectric`
...
**TL;DR**: They derive a self-consistent bidirectional magneto-thermal model for spin caloritronics in which local temperature is dynamic and damping/stochastic magnetic work feed back as localized heat sources; the framework conserves energy and yields correct equilibrium statistics and predicts finite-bath cooling and modified density of states captured by spatially-resolved micromagnetic simulations.  
**Method**: Couple the stochastic Landau–Lifshitz–Gilbert (sLLG) equation to a generalized heat-transfer equation, treat temperature as a dynamical variable, use Itô stochastic calculus to prove thermodynamic consistency and recovery of Boltzmann statistics, and validate with spatially-resolved micromagnetic simulations that resolve energy exchange and DOS modifications under exchange interactions.  
**Tags**: `spin_caloritronics, stochastic_LLG, micromagnetics, magneto-thermal_coupling, nonequilibrium_thermodynamics, heat_transfer, spin_wave_heat_conveyor`
...
**TL;DR**: Nonvolatile magneto-thermal switching in phase-separated Sn/Pb superconducting multilayers appears when micro-scaled Sn inclusions (set by repetition number in accumulative roll bonding) reach sizes comparable to or smaller than the superconducting vortex, enabling magnetic-flux trapping that correlates with remanent magnetization.  
**Method**: Experimental study of Sn/Pb multilayer composites fabricated by accumulative roll bonding with systematic control of individual layer thickness (via repetition number); characterization of magneto-thermal conductivity and remanent magnetization to correlate microstructure (domain size) with the onset of nonvolatile magneto-thermal switching.  
**Tags**: `vortex,superconducting_impurity,quantum_devices,multilayer,flux_trapping`
...
**TL;DR**: Unsupervised nonlinear dimensionality reduction + differential geometry applied to ICSD descriptors reveals a low-dimensional materials manifold that segregates superconductors, distinguishes superconducting families, and enables accurate Tc prediction without input about pairing mechanisms.  
**Method**: Apply differential-geometry analysis and unsupervised nonlinear dimensionality reduction to microscopic descriptors from the Inorganic Crystal Structure Database (ICSD) to obtain a low-dimensional embedding; analyze embedding structure to classify superconductors and correlate embedding coordinates with critical temperatures (Tc) for prediction.  
**Tags**: `quantum_geometry,machine_learning,superconductivity,materials_informatics,dimensionality_reduction,Tc_prediction`
...
**TL;DR**: Driven hybridization between cavity photons and ferromagnetic magnons in a ferromagnet proximitized by an s-wave superconductor can induce a non-equilibrium crossover from a proximitized nodal p-wave state to a fully gapped (p_x+ip_y) superconductor; the crossover temperature scales inversely with magnon–photon detuning and produces measurable, drive-dependent renormalizations of cavity frequencies/kinetic inductance, consistent with features seen in cQED permalloy–niobium experiments.  
**Method**: The authors construct a simplified theoretical model of a ferromagnetic metal proximitized by a fully gapped s-wave superconductor coupled to a microwave resonator. They treat low-energy excitations (magnons, Bogoliubov quasiparticles, cavity photons), include an external drive to generate hybridized driven magnon–polaritons, and compute how this driven hybridization alters the superconducting pairing symmetry (nodal p → p_x+ip_y), determines the crossover temperature (dependent on magnon–photon detuning), and renormalizes cavity photon frequencies/kinetic inductance versus temperature, comparing results to recent cQED experiments.  
**Tags**: `topology, quantum_devices, cQED, magnon-polariton, proximity_effect, driven_non-equilibrium, kinetic_inductance, permalloy-niobium`
...
**TL;DR**: Hf substitution (x=0.02) in ZrTe3 suppresses the 63 K CDW to ~53 K and stabilizes bulk multiband superconductivity (Tc ≃ 3.3 K); Hc2(T) follows a two-band model, Hall/Seebeck indicate enhanced effective masses and strong electronic anisotropy, and hydrostatic pressure shifts T_CDW up while suppressing Tc—suggesting Hf acts as chemical pressure that modifies Fermi-surface topology and correlations.  
**Method**: Experimental study of HfxZr1-xTe3 single crystals (x=0.02): electrical resistivity, magnetic susceptibility, Hall effect, Seebeck coefficient, specific heat, and hydrostatic-pressure (≤ ~2 GPa) measurements; analysis of upper critical field with a two-band model and transport/thermoelectric data to infer carrier anisotropy and effective masses.  
**Tags**: `superconductivity, charge_density_wave, multiband_superconductivity, transport_measurements, pressure_effects, electronic_anisotropy, Fermi_surface, van_der_Waals`
...
**TL;DR**: Mechanical point-contact Andreev reflection spectroscopy on CrSb single crystals with superconducting Nb tips reveals strong, orientation-dependent (k‑dependent) spin polarization — ~73.4% ((0001)), 67.9% ((-1-120)), 61.9% ((10-10)) — and spatially varying conductance consistent with altermagnetic domains (250–500 nm) separated by ≈250 nm domain walls, demonstrating Andreev reflection as a probe of momentum-dependent spin textures in altermagnets.  
**Method**: Mechanical point‑contact spectroscopy (MPCS) using superconducting Nb tips to measure Andreev reflection on CrSb single crystals along three principal crystallographic orientations; extraction of momentum‑dependent spin polarization from conductance spectra and spatial line‑scans to infer domain sizes and domain‑wall widths.  
**Tags**: `altermagnetic, Andreev_reflection, spin_polarization, point_contact_spectroscopy, domain_walls, CrSb, Nb_tip, momentum_dependent_spin_texture`
...
**TL;DR**: In a translationally invariant multiband honeycomb model with Kane–Mele spin–orbit coupling, chiral equal-spin (p-wave) intraband pairing produces a finite anomalous Hall response (and thus a Kerr signal) at optical frequencies despite absence of interband pairing or disorder; chiral d-wave spin-singlet intraband pairing does not. The effect originates from quantum-geometric interband velocity matrix elements and orbital nonunitarity that, with spin–orbit coupling, produce spin-polarized Bogoliubov quasiparticles.  
**Method**: Analytical and numerical evaluation of optical-frequency anomalous Hall conductivity in a honeycomb tight-binding model with Kane–Mele spin–orbit coupling. Compare chiral d-wave spin-singlet and chiral p-wave equal-spin-triplet intraband pairing; identify contributions from Bloch-state quantum geometry (interband velocity matrix elements) and analyze orbital nonunitarity and spin polarization of Bogoliubov states.  
**Tags**: `quantum_geometry,multiband_superconductivity,chiral_superconductivity,spin_triplet,spin_orbit_coupling,anomalous_Hall,polar_Kerr,orbital_nonunitarity,honeycomb_model`
...
**TL;DR**: Combined first-principles, AC calorimetry and directional low-T STM study of tetragonal V2Ga5 (Tc≈3.5 K) finds an anisotropic multiband singlet s-wave state in the A1g representation. A self-consistent multiband pairing model reconciles specific-heat and Hc2 anomalies and shows that the apparent reversal of gap hierarchies in directional tunneling is due to band-selective tunneling controlled by Fermi-velocity "hot spots" and Fermi-surface topology rather than bulk gap magnitudes.  
**Method**: First-principles electronic-structure calculations + sensitive AC calorimetry + directional low-temperature scanning tunneling spectroscopy, combined with a self-consistent anisotropic multiband singlet s-wave (A1g) pairing model and analysis of tunneling matrix-element (Fermi-velocity/FS topology) effects.  
**Tags**: `multiband_superconductivity, anisotropic_s-wave, band-selective_tunneling, directional_STM, Fermi_surface, upper_critical_field, AC_calorimetry, first-principles`
...
**TL;DR**: Study of the 3D current distribution in one-layer conductor-on-round-tube (CORT) superconducting cables showing that the Garber helical current pattern strongly alters AC losses; surface and edge loss contributions are quantified as functions of conductor thickness, pitch angle, and inter-conductor gap.  
**Method**: Parametric numerical study using an effective 2D model built in a coordinate system conforming to the cable’s helical geometry; the model resolves the detailed current distribution in coated conductors and separates surface (parallel-field) and edge (perpendicular-field near gaps) loss contributions.  
**Tags**: `superconductivity,AC_losses,current_distribution,helical_geometry,Garber_current_pattern,effective_2D_model,numerical_modeling`
