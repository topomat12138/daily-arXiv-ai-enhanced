...
**TL;DR**: 3 MeV proton irradiation of HgBa2Ca2Cu3O8+δ (Hg1223) single crystals introduces effective pinning centers and strongly enhances critical current density, peaking at 1×10^16 cm^-2 (self-field Jc(2 K) increased from 5.5 to 26 MA/cm^2). Irradiated samples retain Jc>0.1 MA/cm^2 at 77 K; post‑irradiation Jc(B) follows a power law with exponent α≈1, the normalized magnetic relaxation rate is monotonic in field (linked to low irreversibility field from large anisotropy), and pinning‑force analysis indicates a change in pinning mechanism.  
**Method**: Controlled 3 MeV proton irradiation on Hg1223 single crystals across doses, followed by measurements of self-field critical current density at 2 K and 77 K, magnetic‑field dependence of Jc (power‑law fitting), magnetic relaxation-rate measurements, and pinning‑force density analysis to identify mechanism changes.  
**Tags**: `superconducting_impurity,vortex,pinning,irradiation,cuprate,critical_current`
...
**TL;DR**: JSTM pushed into a deep point-contact (R_N ≃ 0.15 h/2e^2 ≃ 2 kΩ) on AV3Sb5 kagome superconductors shows the zero-bias conductance departs from the expected quadratic scaling with normal-state conductance and instead saturates; the saturation is attributed to series resistance in the circuit, implying caution when interpreting zero-bias conductance saturation/quantization (e.g. for Majorana claims). An optimal intermediate coupling regime is identified for atomic-scale probes of pair-density-wave states in low-Tc materials.  
**Method**: Experimental Josephson scanning tunneling microscopy in the deep point-contact regime on kagome superconductors (AV3Sb5); systematic measurement of zero-bias conductance versus normal-state junction conductance, analysis of scaling deviations and identification of series-resistance-limited saturation; determination of optimal coupling regime for detecting pair-density waves.  
**Tags**: `topology, quantum_devices, superconductivity, Josephson_scanning_tunneling_microscopy, point_contact_regime, kagome_superconductors, pair_density_wave, zero-bias_conductance, series_resistance, Majorana`
...
**TL;DR**: Thickness-dependent superconductivity in ultra-thin LaBi2 films is analyzed with a multi-mechanism Kharitonov–Feigel’man framework. By resolving field‑enhanced superconductivity in the thin limit the authors quantify the contribution of spin‑exchange (exchange) scattering in addition to paramagnetic and orbital depairing, and reinterpret how Tc, the Pauli limit, and scattering times should be defined in 2D superconductors.  
**Method**: Combine thickness-dependent measurements of superconducting response under in-plane magnetic fields with analysis using the multi-mechanism Kharitonov–Feigel’man theoretical framework to separate and quantify depairing channels (paramagnetic, orbital, and spin‑exchange scattering).  
**Tags**: `superconductivity, superconducting_impurity, Pauli_limit, thin_films, spin-exchange-scattering, paramagnetic_effect, orbital_effects, Kharitonov-Feigelman`
...
### [4] [Massive Mitigation of Transport AC Losses in Superconducting Hybrid CORC-TSTC Cables](https://arxiv.org/abs/2605.05981)
*Hasan N. Al-Ssalih, Antonio Badía-Majós, Harold S. Ruiz*

**Category**: cond-mat.supr-con  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.05981)

**TL;DR**: Independent (insulated) current feeding of hybrid CORC–TSTC superconducting cables strongly suppresses inter-conductor current redistribution, stabilises current waveforms and reduces transport AC losses by up to ~90% versus common (non‑insulated) feeding; demonstrated in self‑field using a validated fully‑3D electromagnetic model.  
**Method**: Comparative numerical study using a fully‑3D electromagnetic simulation (previously validated against magnetisation experiments) of hybrid CORC–TSTC cables under self‑field; examines non‑insulated (common) vs insulated (independent) current injection to quantify internal current redistribution, waveform distortion, magnetic saturation of CORC layers and resulting transport AC losses.  
**Tags**: `superconductivity, AC_losses, CORC, TSTC, hybrid_superconducting_cables, current_injection, electrical_coupling, electromagnetic_model, transport_losses`
...
### [5] [Thermodynamic incompleteness in non-Markovian Majorana transport I: Island dynamics and missing transport statistics](https://arxiv.org/abs/2605.05754)
*Yang Tian*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.05754)

**TL;DR**: Island-state non-Markovian dynamics in a floating Coulomb-blockaded Majorana island do not in general determine lead-resolved thermodynamic transport statistics. In the cotunneling regime, tracing out structured reservoirs yields island memory kernels that suffice to predict island observables but erase which-reservoir information (which lead/detector supplied or absorbed electrons and energy). The authors state a thermodynamic completeness criterion and show two devices can have identical island tomography yet different lead charge/heat noise and mixed correlations; the missing information is tied to the projection geometry and tunnel-contact network topology.  
**Method**: Analytical theoretical study in the cotunneling (Coulomb-blockade) regime: perform a Schrieffer–Wolff transformation to derive reservoir-assisted transitions generated by Majorana bilinears, trace out structured reservoirs to obtain non-Markovian island memory kernels and a memory equation, and formulate a thermodynamic-completeness criterion relating island kernels to measurable transport observables; identify measurable consequences (lead charge/heat noise and mixed correlations) and relate them to the projection geometry and tunnel-contact network topology.  
**Tags**: `quantum_devices,topology,quantum_computing,superconducting_impurity,non-Markovian,cotunneling,Majorana_zero_modes,transport_noise,thermodynamic_statistics`
...
**TL;DR**: New type of quantum oscillations in the second-order nonlinear Hall effect is proposed and experimentally observed in graphene moiré devices. The effect arises from alternation of dominant NLHE mechanisms among recurring Bloch states under a perpendicular magnetic field, allowing sensitive detection of Brown–Zak fermions (onset ~0.5 T). At magnetic commensurability, the nonlinear response is dominated by quantum-geometric (topological) contributions, enabling the first experimental probe of the topological nature of Brown–Zak fermions.  
**Method**: Combined theoretical proposal and transport measurements in graphene moiré systems: identify NLHE quantum oscillations produced by mechanism-switching of nonlinear Hall contributions with recurring Bloch states under magnetic field; experimental nonlinear transport detects Brown–Zak oscillations and establishes quantum-geometric-dominated response at commensurability.  
**Tags**: `quantum_geometry, topology, nonlinear_Hall_effect, quantum_oscillations, graphene_moiré, Brown-Zak_fermions, quantum_devices`
...
### [7] [Emergent spin quantum Hall edge states at the boundary of two-dimensional electron gas proximitized by an $s$-wave superconductor](https://arxiv.org/abs/2605.05847)
*M. V. Parfenov, V. S. Khrapai, I. S. Burmistrov*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.05847)

**TL;DR**: In 2DEG–superconductor hybrids under quantizing magnetic field the authors argue topological protection of chiral Andreev edge states manifests in spin transport: the system is in Altland–Zirnbauer class C and supports an even-integer quantized transverse spin conductivity (spin quantum Hall effect) that is robust to disorder. They theoretically demonstrate protected spin-carrying edge modes and propose an electrical measurement to detect this class-C origin.  
**Method**: Symmetry-class analysis (Altland–Zirnbauer class C) combined with theoretical transport arguments/calculations showing even-integer quantized transverse spin conductance for chiral Andreev edge states; plus a proposed experimental electrical probing protocol.  
**Tags**: `topology, superconductivity, spin_quantum_Hall, class_C, Andreev_edge_states, disorder, quantum_devices`
...
**TL;DR**: The paper identifies the Dzyaloshinskii–Moriya (DM) interaction in superexchange through a chiral molecular bridge as a binary, symmetry-based order parameter that distinguishes coherent (unitary spin-rotation) CISS from incoherent (dissipative/spin-filtering) scenarios. Coherent CISS generates a giant DM term (|D|/J_H up to ~3); any Hermitian, spin-diagonal (incoherent) tunneling yields D=0 exactly. The authors derive analytical angular/enantiomeric signatures, provide a Lindblad-based proof, estimate experimental thresholds well within existing exchange-spectroscopy sensitivity, and identify molecular candidates exceeding the threshold.  
**Method**: Analytical and theoretical derivation: model tunneling through a chiral bridge as either unitary spin rotation or Hermitian spin-diagonal transfer; compute virtual-tunneling-mediated superexchange to extract Heisenberg (J_H) and DM (D) terms; prove a structural theorem (D=0 for incoherent case) and reinforce it with a Lindblad open-quantum-system argument; derive closed-form angular/enantiomeric signatures and quantitative estimates for exchange-spectroscopy detection; screen candidate molecules and estimate interface amplification.  
**Tags**: `CISS, Dzyaloshinskii-Moriya, superexchange, chiral_molecules, Lindblad_dissipation, exchange_spectroscopy, quantum_devices, quantum_computing`
...
### [9] [Anomalous Thomson Effect](https://arxiv.org/abs/2605.06299)
*Ying-Fei Zhang, Zhi-Fan Zhang, Zhen-Gang Zhu, Gang Su*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.06299)

**TL;DR**: Proposes the anomalous Thomson effect (ATE) and derives an anomalous Thomson coefficient (ATC) in terms of the anomalous Nernst coefficient (ANC), showing ATC inherits Berry-curvature mechanisms and is generally larger than ANC (ATC/ANC→3 at low T). Using a massive Dirac model for Fe3Sn2 and a model-independent relation, experimental ANE data for CoS2, Co3Sn2S2 and CeCrGe3 are used to infer ATC; CeCrGe3 may exhibit ATC up to ~15×ANC near liquid-nitrogen temperatures, suggesting promise for solid-state thermoelectric refrigeration.  
**Method**: Analytical derivation linking ATC to ANC (model-independent relation) combined with microscopic calculation using a massive Dirac model to capture intrinsic Berry-curvature–driven transport in Fe3Sn2, and extraction of ATC estimates from existing experimental ANE data for several materials.  
**Tags**: `quantum_geometry,topology,berry_curvature,anomalous_nernst,anomalous_thomson,thermoelectrics,Fe3Sn2,Co3Sn2S2,CeCrGe3`
...
### [10] [Twisted Kagome Bilayers: Higher-Order Magic Angles, Topological Flat Bands, and Sublattice Interference](https://arxiv.org/abs/2605.06551)
*David T. S. Perkins, Joseph J. Betouras*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.06551)

**TL;DR**: Continuum generalization of the Bistritzer–MacDonald approach for moiré heterostructures applied to twisted bilayer kagomé near 1/3 filling. The model predicts higher-order ‘magic’ twist angles with strong local band flattening associated with high-order Van Hove singularities, shows twist-induced nontrivial band topology, and finds sublattice interference is present but less decisive than in monolayer kagomé.  
**Method**: Develop a low-energy continuum (BM-like) moiré model for heterostructures and apply it to twisted bilayer kagomé (TBK) near monolayer Dirac points at 1/3 filling; analyze moiré bandstructure to identify magic angles, local flattening, high-order Van Hove singularities, and emergent topology.  
**Tags**: `topology, quantum_geometry, moire, twisted_bilayer, kagome, magic_angles, van_hove, continuum_model`
...
### [11] [Tunable Interlayer Charge-transfer States in MoSe$_2$/WS$_2$ Moiré Superlattices](https://arxiv.org/abs/2605.05571)
...
**TL;DR**: Angle-aligned MoSe2/WS2 moiré heterobilayers (electron-doped) host tunable moiré excitons and interlayer charge-transfer states: combining first-principles calculations and optical reflection spectroscopy the authors show that moiré excitons probe electron localization, report successive interlayer charge-transfer transitions for fillings n/n0=1–4 when an applied vertical field switches band alignment from Type-I to Type-II, and demonstrate control over a charge-transfer band realizing a tunable Fermi–Hubbard model on an effective honeycomb lattice; Monte Carlo predicts multiple correlated charge-ordered states at integer and fractional fillings.  
**Method**: Large-scale first-principles calculations (first-principles electronic structure), optical reflection spectroscopy on gated, angle-aligned MoSe2/WS2 heterobilayers with vertical electric-field tuning, and Monte Carlo simulation of doping-dependent electric-field susceptibility.  
**Tags**: `moiré, transition_metal_dichalcogenide, excitons, interlayer_charge_transfer, electron_doping, Fermi-Hubbard, moiré_superlattice, optical_spectroscopy, Monte_Carlo, charge_order, honeycomb_lattice, quantum_geometry`
...
**TL;DR**: Floquet engineering of cavity magnon-polaritons is realized by modulating the microwave cavity frequency (rather than the magnon bias field). Using commensurate two-frequency drives (higher tone at 2× and 3× the lower tone) they show the Floquet quasi-energy spectrum depends on relative amplitude and phase and exhibits new anticrossings between previously uncoupled sidebands—providing an alternative knob to control hybrid magnon–photon spectra.  
**Method**: Implement cavity-frequency Floquet modulation with commensurate two-tone periodic drives and analyze the resulting magnon–photon hybrid (Floquet quasi-energy) spectrum; compare to single-frequency Floquet modulation and identify amplitude- and phase-dependent anticrossings between sidebands.  
**Tags**: `floquet_engineering,cavity_magnon-polariton,driven_cavity,hybrid_magnon-photon,sideband_anticrossings,quantum_devices`
...
**TL;DR**: Low-temperature STM/STS reveals quantized collective (bosonic) spin and charge modes in atomically confined mirror-twin-boundary (MTB) segments of monolayer WSe2; Luttinger-liquid-like spin–charge separation persists down to ≈1 nm, turning segments into many-body ‘quantum dots’ whose discrete spectra arise from confined collective modes. Ordered chains show modified electron-like fundamental states while fractionalized excitations remain robust, demonstrating atomic-scale survival of collective quantum matter and a platform for engineering correlated phases.  
**Method**: Low-temperature scanning tunneling microscopy and spectroscopy on atomically confined mirror-twin-boundary segments in monolayer WSe2, mapping standing-wave branches attributed to fractionalized spin and charge excitations in isolated segments (down to ~1 nm) and in assembled ordered chains to probe inter-dot coupling effects.  
**Tags**: `Tomonaga-Luttinger_liquid,spin-charge_separation,1D_many-body,STM_STS,mirror_twin_boundary,WSe2,many-body_quantum_dot,collective_bosonic_modes,inter-dot_coupling,quantum_devices`
...
**TL;DR**: Using Schwinger–Keldysh field theory, the authors integrate out driven altermagnetic magnons to obtain an extended Landau–Lifshitz–Gilbert equation for a ferromagnetic-insulator layer. The effective LLG contains two spatially nonlocal, anisotropic damping terms (one non‑Markovian in time), allowing control of spin-wave, domain-wall and skyrmion dynamics via the driven magnon bath.  
**Method**: Schwinger–Keldysh field-theory treatment of a driven/dissipative bosonic bath (quantum magnons in an altermagnetic insulator) coupled to classical localized spins in a ferromagnetic-insulator layer; derivation of an effective, extended LLG equation with spatially nonlocal and temporally nonlocal (non‑Markovian) damping terms.  
**Tags**: `altermagnetic, spintronics, magnonics, Schwinger-Keldysh, LLG_equation, nonlocal_damping, non-Markovian, skyrmions, domain_walls, spin_waves`
