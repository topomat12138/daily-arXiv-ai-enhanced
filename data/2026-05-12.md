<div id=toc></div>

# Table of Contents

- [cond-mat.mes-hall](#cond-mat.mes-hall) [Total: 15]
- [cond-mat.supr-con](#cond-mat.supr-con) [Total: 3]


<div id='cond-mat.mes-hall'></div>

# cond-mat.mes-hall [[Back]](#toc)

### [1] [Spin Seebeck effect in magnetic junctions with a compensated ferrimagnet](https://arxiv.org/abs/2605.09885)
*Xin Theng Lee, Takahiro Misawa, Mamoru Matsuo, Takeo Kato*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09885)

**TL;DR**: Compensated ferrimagnets with sublattice inequivalence from exchange-coupling asymmetry produce a robust spin Seebeck current (via isotropic magnon band splitting) comparable to ferromagnets, while altermagnets under identical conditions yield no spin Seebeck effect. This identifies exchange-asymmetric compensated ferrimagnets as stray-field-free thermal spin-current sources for spintronics.  
**Method**: Analytical and numerical study using a four-sublattice model with exchange-coupling inequivalence; spin transport computed within a nonequilibrium Green’s function framework, focusing on isotropic magnon splitting and comparison with altermagnet cases.  
**Tags**: `altermagnetic, compensated_ferrimagnet, spin_Seebeck, spin_caloritronics, magnonics, nonequilibrium_greens_function, quantum_devices`

<details>
  <summary>Abstract</summary>
Compensated ferrimagnets enable ferromagnet-like spin transport without net magnetization. We study the spin Seebeck effect in a compensated ferrimagnet/normal-metal junction using a four-sublattice model in which sublattice inequivalence arises from differences in exchange couplings, in contrast to the previously studied anisotropy-based mechanism. Within the nonequilibrium Green's function framework, we show that isotropic magnon splitting generates a robust spin current with a magnitude comparable to that in standard ferromagnetic junctions. We also demonstrate that the spin Seebeck effect vanishes in altermagnet junctions under identical conditions, thereby establishing compensated ferrimagnets as uniquely suited for thermal spin-current generation among magnetically compensated systems. These results provide a theoretical basis for the applications of compensated ferrimagnets with exchange-coupling asymmetry as stray-field-free spin-current sources in spintronic devices.
</details>


### [2] [Cascade of fractional quantum Hall states in 2D system](https://arxiv.org/abs/2605.10147)
*Chen Zhimou, Yan Jiaojie, Zhu Yuxuan, Cui Zhe, Pfeiffer Loren N., West Kenneth W., Baldwin Kirk W., Gupta Adbhut, Liu Yang, Zhu Wei, Luo Wenchen, Wu Ying-Hai, Yuan Shuai, Lin Xi*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10147)

**TL;DR**: Ultrahigh-mobility GaAs/AlGaAs quantum wells (μ up to 3.7×10^7 cm^2/Vs) measured down to 1 mK show many known fractional quantum Hall (FQH) states and new longitudinal-resistance minima at ν=17/33 and 15/31. The authors probe the effect of in-plane magnetic field, and interpret the full set of observed fractions (0<ν<2) within composite-fermion (CF) theory, identifying most as integer quantum Hall states of noninteracting CFs and a subset as FQH states of CFs due to residual CF–CF interactions; they propose an empirical pattern for the relative strengths of the fractions.  
**Method**: Low-temperature quantum transport (longitudinal resistance) on ultrahigh-mobility GaAs/AlGaAs quantum wells, measured in nuclear adiabatic demagnetization and dilution refrigerators down to 1 mK; tunable in-plane magnetic field; theoretical interpretation using composite-fermion theory to classify observed fractions as IQH of CFs or FQH of CFs arising from residual interactions.  
**Tags**: `topology,fractional_quantum_Hall,composite_fermion,high_mobility_GaAs,low_temperature,quantum_transport,quantum_geometry`

<details>
  <summary>Abstract</summary>
The observation of the fractional quantum Hall (FQH) effect in 2D electron gases ushered in investigations of topological phases driven by strong electron correlations. Their remarkable features include fractionalized elementary excitations, gapless boundary states, and non-trivial quantum entanglement patterns. Thanks to persistent efforts in the building of new platforms and making higher-quality samples, a diverse plethora of FQH states have been unveiled in experiments. We report a systematic study of ultrahigh-quality GaAs/AlGaAs quantum wells with mobility up to 3.7*10^7 cm^2/V/s using quantum transport measurements in nuclear adiabatic demagnetization and dilution refrigerators down to 1 mK. In addition to many FQH states that have already been identified in previous work, new longitudinal resistance dips are observed at filling factors 17/33 and 15/31. The application of an in-plane magnetic field causes disparate variations of the FQH states. The theoretical foundation of these states is discussed in the framework of composite fermion theory. While most fractions can be explained as non-interacting composite fermions forming integer quantum Hall states, a few states correspond to FQH states of composite fermions that arise from residual interaction between them. We summarize the observed fractions in the range of 0 < ν < 2 and propose a pattern to account for their experimental appearance that provides an intuitive picture about the relative strengths of different FQH states.
</details>


### [3] [Ultra-Fast Quantum Control via Non-Adiabatic Resonance Windows: A 9x Speed-up on 127-Qubit IBM Processors](https://arxiv.org/abs/2605.10578)
*A. M. Tishin*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10578)

**TL;DR**: Discovery of a universal non-adiabatic resonance window (parameter ≈ 4.9) on IBM 127‑qubit superconducting hardware that enables up to 9.2× faster gates than conventional adiabatic protocols while retaining high fidelities; the resonance profile is reproducible across backends (R=0.9998) but is fragile to sub‑percent calibration drifts, motivating dynamic resonance‑tracking control.  
**Method**: Experimental characterization and theoretical support: synchronous cross‑backend execution on IBM Quantum 127‑qubit processors (ibm_fez, ibm_kingston) to map a non‑adiabatic resonance window and compare gate durations/fidelities versus adiabatic limits, plus longitudinal drift analysis to assess sensitivity and dynamical behavior; authors claim a theoretical foundation underpinning the observed resonance.  
**Tags**: `quantum_computing, quantum_devices, superconducting_qubits, non-adiabatic_control, gate_speed, resonance, calibration_drift, dynamic_control`

<details>
  <summary>Abstract</summary>
Standard adiabatic protocols for superconducting qubits often face a trade-off between gate speed and decoherence. In this work, using IBM Quantum 127-qubit processors (ibm_fez and ibm_kingston), we report the discovery of a fundamental non-adiabatic resonance window at about 4.9. This window demonstrates the potential for a 9.2-fold reduction in gate duration relative to the conventional adiabatic limit, while maintaining state high fidelities within the identified resonance windows. Through synchronous cross-backend execution, we demonstrate a near-perfect correlation (R = 0.9998) in the resonance profile, confirming the universality of the non-adiabatic parameter across independent hardware architectures. However, our longitudinal analysis reveals that these high-Q windows are sensitive to sub-percent calibration drifts, which dynamically shift the system into a stochastic regime. These findings suggest that achieving next-tier quantum performance requires a transition from static gate protocols to dynamic resonance-tracking control tools. This study provides both the theoretical foundation and the experimental evidence for such ultra-fast, high-performance quantum architectures.
</details>


### [4] [Inherent Altermagnetism on regular hyperbolic lattices](https://arxiv.org/abs/2605.10602)
*Eric Petermann, Kristian Mæland, Haye Hinrichsen, Björn Trauzettel*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10602)

**TL;DR**: Extends Euclidean tight‑binding altermagnet models to regular 2D hyperbolic lattices on a discretized Poincaré disk and finds that including next‑nearest‑neighbor hopping generates momentum‑dependent spin splitting (altermagnetism) in an entire family and a special case of bipartite hyperbolic lattices. Because hyperbolic band theory yields a momentum space of at least four dimensions, leading spin‑splitting harmonics are classified using four‑dimensional atomic‑orbital harmonics.  
**Method**: Construct tight‑binding Hamiltonians on discretized Poincaré‑disk hyperbolic lattices; apply hyperbolic crystallography and hyperbolic band theory to obtain momentum‑space bands; introduce next‑nearest‑neighbor hopping to induce spin splitting; classify resulting spin‑splitting harmonics using a four‑dimensional atomic‑orbital basis.  
**Tags**: `altermagnetic, hyperbolic_band_theory, quantum_geometry, tight_binding, spin_splitting, bipartite_lattices`

<details>
  <summary>Abstract</summary>
Altermagnets are a novel class of magnetic systems characterized by their momentum-dependent spin splitting without net magnetization. In this work, we extend established Euclidean tight-binding models of altermagnets to regular hyperbolic lattices in two spatial dimensions defined on a discretized Poincaré disk. Using hyperbolic crystallography and hyperbolic band theory, we show that the inclusion of next-nearest neighbor hopping is sufficient to induce spin splitting in bipartite hyperbolic lattices. While certain families and special cases of hyperbolic lattices remain antiferromagnetic, we identify an entire family and a special case that show spin splitting in this framework. Hence, altermagnetism is inherent to certain hyperbolic lattices. Since hyperbolic band theory yields a momentum space that is at least four-dimensional, we classify the leading spin-splitting harmonics using four-dimensional atomic orbitals.
</details>


### [5] [Theory and Experiment of Chirality-induced Magnetic Nonreciprocity Manifested by Coupling Phase](https://arxiv.org/abs/2605.09135)
*Jiguang Yao, Ying Yang, Chenyang Lu, Lihua Zhong, Xiaolong Fan, Desheng Xue, C. -M. Hu*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09135)

**TL;DR**: Unified experimental and theoretical study of magnetic nonreciprocity that contrasts conventional chiral electromagnetic-field mechanisms with ‘synthetic chirality’ arising from nontrivial phase accumulation in traveling-wave–mediated couplings. The authors develop a microscopic framework that maps field polarization onto the phase of a complex coupling constant and validate it with systematic experiments, emphasizing symmetry properties that distinguish synthetic chirality from conventional mechanisms.  
**Method**: Combined microscopic theoretical modeling and systematic experiments: derive a formal mapping between field polarization and the phase of a complex coupling strength (capturing both conventional chiral fields and traveling-wave–mediated synthetic chirality), and validate the formalism experimentally while analyzing symmetry constraints.  
**Tags**: `nonreciprocity, synthetic_chirality, chiral_photonics, photon-magnon, traveling-wave_coupling, magnetic_symmetry, microscopic_theory, quantum_devices`

<details>
  <summary>Abstract</summary>
Magnetic interactions have long served as the most robust and widely used approach for realizing nonreciprocity, with an externally applied magnetic field breaking time-reversal symmetry (TRS) and chiral photon-magnon interactions introducing spatial asymmetry. In this work, we investigate the chirality mechanisms essential for magnetic nonreciprocity from a unified experimental and theoretical perspective. We begin by examining conventional chiral interactions that generate chiral electromagnetic fields through specially designed structures, and then place particular emphasis on synthetic chirality enabled by nontrivial phase accumulation in traveling-wave-mediated coupling systems. We establish a microscopic theoretical framework that maps field polarization onto the phase of a complex coupling strength and validate it with systematic experiments, thereby providing a consistent formalism that describes both conventional and synthetic chirality. Notably, we highlight the symmetry properties and the unique features of synthetic chirality that distinguish it from conventional nonreciprocal mechanisms.
</details>


### [6] [Magnetization alignment in spin-transfer-torque magnetic random-access memory](https://arxiv.org/abs/2605.09201)
*Afan Terko, George Lertzman-Lepofsky, Dieter Suess, Claas Abert, Erol Girt*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09201)

**TL;DR**: Micromagnetic simulation study of 30 nm, three-layer perpendicular STT-MRAM nanopillars maps equilibrium states versus bilinear and biquadratic interlayer exchange. Introducing asymmetry between SAF layers (Ms, anisotropy, thickness) lowers coupling needed to stabilize antiparallel SAF states and suppresses competing configurations; asymmetry raises SAF reversal barriers but can lower the free-layer barrier for noncollinear antiparallel states (trade-off absent for collinear states). Stray fields substantially affect energy barriers. The authors release a 4374-configuration simulation dataset.  
**Method**: Systematic micromagnetic simulations with experimentally motivated material parameters; construction of phase diagrams over bilinear and biquadratic interlayer exchange; minimum-energy-path calculations to extract reversal barriers; parameter sweeps introducing SAF asymmetry and inclusion of stray-field effects; public release of the full simulation dataset.  
**Tags**: `quantum_devices, spintronics, STT-MRAM, synthetic_antiferromagnet, micromagnetics, interlayer_exchange, minimum_energy_path, energy_barriers, stray_fields`

<details>
  <summary>Abstract</summary>
Reliable operation of perpendicular spin-transfer-torque magnetic random-access memory (p-STT-MRAM) requires control of magnetic alignment within the synthetic antiferromagnet (SAF) reference layer. At nanopillar dimensions, however, devices can exhibit magnetic states that are absent in extended thin films. We present a systematic micromagnetic study of 30 nm-diameter three-layer p-STT-MRAM nanopillars using experimentally motivated material parameters, and map equilibrium states as functions of bilinear and biquadratic interlayer exchange coupling. Phase diagrams show that introducing asymmetry between the SAF layers in saturation magnetization, anisotropy, and thickness reduces the coupling strength required to stabilize antiparallel SAF states and suppress competing configurations. Minimum-energy path calculations show that, for noncollinear antiparallel SAF states, increasing SAF asymmetry can raise SAF reversal barriers while lowering the free-layer barrier; this trade-off is absent for collinear antiparallel SAF states. Stray fields also significantly modify both SAF and free-layer energy barriers. To support the design of p-STT-MRAM devices with either collinear or noncollinear antiparallel SAF reference states, we publicly release the simulation dataset covering 4374 distinct device configurations.
</details>


### [7] [Spin Elasticity:A New Paradigm for Spintronics](https://arxiv.org/abs/2605.09240)
*Zhong-Chen Gao, Tianyi Zhang, Feifei Wang, Jingguo Hu, Peng Yand, Xiufeng Han*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09240)

**TL;DR**: Proposes a theoretical framework called “spin elasticity” that links spin torque to spin morphology, yielding a topological analogue of Hooke’s law, predicting spontaneous oscillations/resonances and a new class of collective modes — “spin stress waves.” Introduces a unified tau–D theory connecting classical elasticity and topological spin physics with implications for spintronics.  
**Method**: Analytical/theoretical construction: define a spin-elasticity framework mapping spin torque to morphological strain, derive a topological Hooke’s law, and develop a unified tau–D theory to predict dynamical phenomena (spontaneous oscillations, resonance) and new collective excitations (spin stress waves).  
**Tags**: `topology, spintronics, elasticity, collective_excitations, spin_waves, topological_hooke_law, tau-D_theory, spin_elasticity, topological_spin_physics`

<details>
  <summary>Abstract</summary>
Elasticity shapes our world. For centuries, it has been regarded as a property exclusive to ordinary matter. Here we uncover its hidden existence in the spin degree of freedom. We introduce spin elasticity-a framework linking spin torque to spin morpgology. This reveals a topological Hooke's law, uncovers spontaneous oscillations and resonance, and predicts a new class of collective excitations:spin stress waves. By establishing a unfied tau-D theory bridging classical elasticity and topological spin physics, this work completes the elastic picture and opens a new frontier for spintronics-spinelastronics.
</details>


### [8] [Coherence, long-range transport and nuclear polarization in a driven-dissipative dark exciton condensate](https://arxiv.org/abs/2605.09488)
*Amit Jash, Maheswar Swar, Uri Shimon, Vladimir Umansky, Israel Bar-Joseph*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09488)

**TL;DR**: Direct evidence of a macroscopic, long-lived dark-exciton condensate in coupled quantum wells is reported; condensation is governed by driven-dissipative gain–loss selection favoring dark states, produces millimeter-scale hydrodynamic transport and coherence, and couples to nuclear spins via Overhauser-field-induced closing/locking of the dark–bright gap with hysteresis and a second photoluminescence threshold.  
**Method**: Experimental optical study of coupled quantum wells using spatially-resolved photoluminescence (darkening, blueshift, changes in radiative channels), interferometry between incident and boundary-reflected exciton currents to extract coherence length, observation of long-range propagation of density (sound) modes, and detection of dynamic nuclear polarization (Overhauser field) to track gap closing and hysteresis.  
**Tags**: `excitons, dark_excitons, exciton_condensate, driven_dissipative, coupled_quantum_wells, hydrodynamic_transport, dynamic_nuclear_polarization, coherence_length, quantum_devices`

<details>
  <summary>Abstract</summary>
We report direct evidence for macroscopic coherence in a condensate of dark dipolar excitons in coupled quantum wells and show that its formation follows a non-equilibrium, driven-dissipative mechanism. The condensation transition is governed by gain-loss competition, in which the exceptionally long lifetime of dark excitons enables their dominance in mode selection. Condensate formation is revealed by photoluminescence darkening, changes in radiative recombination channels, and the emergence of long-range hydrodynamic transport manifested by propagation of density (sound) modes over millimeter-scale distances. The buildup of dark exciton density induces dynamic nuclear polarization, which closes the dark-bright exciton gap, Δ, via the Overhauser field. This leads to nuclear spin polarization across the entire mesa, far beyond the optically excited region, and produces pronounced hysteresis behavior. At Δ~ 0 the gap is locked and the condensate loss are minimal, resulting in a second threshold manifested as a photoluminescence blueshift. Coherence is revealed through interference between incident and boundary-reflected exciton currents, producing spatial modulation of the photoluminescence from the radiative reservoir and enabling extraction of the condensate coherence length. These results establish dark excitons as a platform for coherent quantum fluids in a driven-dissipative, strongly interacting regime with electrical tunability, bridging the physics of polariton condensates and matter-like excitonic systems.
</details>


### [9] [Purcell enhancement in layered InSe on the Mie-resonant silicon nitride waveguide](https://arxiv.org/abs/2605.09645)
*A. I. Veretennikov, E. A. Shepelev, A. S. Shorokhov, P. A. Alekseev, I. A. Eliseyev, A. A. Fedyanin, M. V. Rakhlin*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09645)

**TL;DR**: Integration of a thin InSe van der Waals flake with a Mie-resonant Si3N4 waveguide yields Purcell-enhanced spontaneous emission: time-resolved PL shows up to 3× faster exciton decay compared to planar InSe, with extracted Purcell factors ≈3 (out-of-plane) and ≈2.1 (in-plane).  
**Method**: Fabrication and optical characterization of a Mie-resonant Si3N4 waveguide coupled to a thin InSe flake; spectral design to overlap the InSe photoluminescence band; time-resolved photoluminescence to measure excitonic decay and extract Purcell factors for different dipole orientations.  
**Tags**: `quantum_devices, excitons, Purcell_effect, van_der_Waals, dielectric_resonator, Mie_resonance, integrated_photonics, InSe`

<details>
  <summary>Abstract</summary>
Hybrid integration of layered van der Waals (vdW) semiconductors with dielectric resonant structures provides an effective approach for controlling excitonic emission dynamics. Here, we demonstrate Purcell-enhanced spontaneous emission from a thin InSe flake integrated with a Mie-resonant Si$_3$N$_4$ waveguide. The structure is designed to spectrally overlap with the InSe photoluminescence band and enhance coupling of excitonic emission to the guided mode. Time-resolved photoluminescence shows a reduction of the excitonic decay time by up to a factor of three relative to planar InSe. The extracted Purcell factors are approximately 3 for out-of-plane excitons and 2.1 for in-plane excitons. These results demonstrate resonator-induced control of excitonic recombination in layered InSe and highlight vdW-dielectric interfaces as a platform for integrated excitonic and quantum photonic devices.
</details>


### [10] [Antisymmetric linear transverse magnetization and ferroaxial moments induced by geometry-driven electric field gradients](https://arxiv.org/abs/2605.09966)
*Akane Inda, Satoru Hayami*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09966)

**TL;DR**: Finite-system geometry produces electric-field gradients that induce transverse magnetization and ferroaxial moments; these responses (enhanced by tuning trapezoid leg inclination) are antisymmetric and linear in magnetic field and scale linearly with applied electric field (vs quadratic longitudinal response), enabling controllable unconventional transverse responses in mesoscopic systems.  
**Method**: Linear-response Kubo formalism combined with real-time numerical simulations of a finite trapezoidal model; analysis of symmetry (Onsager reciprocity) and parameter variation by tuning leg inclination to control gradient strength.  
**Tags**: `ferroaxial,transverse_magnetization,electric_field_gradient,mesoscopic_systems,Kubo_formalism,linear_response,real-time_simulation,symmetry_breaking,quantum_devices`

<details>
  <summary>Abstract</summary>
We theoretically investigate the transverse magnetization and ferroaxial moments induced by electric field gradients arising from the geometry of finite systems. Based on the Kubo formalism and real-time numerical simulations for a finite trapezoidal model, we demonstrate that both quantities are generated under the electric field gradient and are enhanced by tuning the leg inclination, which controls the gradient strength. We further show that the induced transverse magnetization is antisymmetric and linear in the magnetic field; such a response is prohibited by Onsager reciprocity in the absence of an electric field gradient. In addition, we find that the total transverse magnetization scales linearly with the electric field, in contrast to the longitudinal one, which exhibits a quadratic dependence, providing an advantage for experimental observation. Our results establish geometry-induced electric field gradients as a versatile mechanism for realizing and controlling unconventional transverse responses in mesoscopic systems.
</details>


### [11] [Orbital and Spin Nernst Effects in Monolayers of Transition Metal Dichalcogenides](https://arxiv.org/abs/2605.10033)
*Saikat Saha, Arnab Bose, Sayantika Bhowal*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10033)

**TL;DR**: Predicts and analyzes the orbital Nernst effect in monolayer transition-metal dichalcogenides (TMDs): orbital Nernst exists without spin–orbit coupling (SOC), SOC induces an additional spin Nernst response proportional to SOC strength. Analytical low-energy valley modeling and full Brillouin-zone tight-binding calculations (MoS2, NbS2) identify orbital and spin Berry curvatures as the driving mechanisms; doping controls the effect in semiconducting MoS2 while metallic NbS2 hosts intrinsic responses. A possible experimental detection geometry is proposed.  
**Method**: Analytical low-energy valley model to identify mechanisms plus full Brillouin-zone tight-binding calculations of orbital and spin Nernst conductivities (including Berry-curvature analysis) for monolayer 2H MoS2 and 2H NbS2; study of SOC inclusion and carrier doping dependence.  
**Tags**: `quantum_geometry,orbital_Nernst,spin_Nernst,Berry_curvature,transition_metal_dichalcogenides,tight_binding,valley_model,thermoelectric,doping`

<details>
  <summary>Abstract</summary>
In recent years, orbitronic effects have attracted growing attention as complementary counterparts to the well-established spintronic phenomena. In this work, we demonstrate that monolayers of transition metal dichalcogenides provide an excellent platform for the observation of the orbital Nernst effect, a relatively less explored phenomenon describing the generation of a transverse orbital current in response to an applied temperature gradient. We show that, similar to its electrical counterpart, viz., the orbital Hall effect, the orbital Nernst effect does not require the presence of spin-orbit coupling. Analytical results based on a low-energy valley model offer key insights into the underlying mechanisms, highlighting in particular the crucial role of electronic states at the Fermi energy for the emergence of this effect. The inclusion of spin-orbit coupling further gives rise to a spin Nernst effect, which scales with the strength of spin-orbit coupling and vanishes in its absence. We substantiate our analytical findings with full Brillouin-zone tight-binding results for two representative systems, monolayer 2H MoS$_2$ and 2H NbS$_2$. Our results show that while both orbital and spin Nernst conductivities in MoS$_2$ require electron or hole doping, both effects are intrinsically present in metallic NbS$_2$. Our work reveals the central role of orbital and spin Berry curvatures, identifies doping as an effective route for tuning orbital and spin Nernst responses, and proposes a possible experimental setup for detecting these effects in monolayer transition metal dichalcogenides.
</details>


### [12] [Theory of Spin-splitter Magnetoresistance in Altermagnets](https://arxiv.org/abs/2605.10747)
*Tim Kokkeler, Vitaly N. Golovach, F. Sebastian Bergeret*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10747)

**TL;DR**: Develops an analytical theory of angle-dependent magnetoresistance (ADMR) for metallic altermagnets proximitized by ferromagnetic insulators and shows that the observed spin-splitter magnetoresistance (SSMR) is a definitive transport signature of collinear altermagnetism. It identifies three distinguishing experimental features that separate SSMR from conventional spin-Hall magnetoresistance (SMR).  
**Method**: Analytical/theoretical derivation of ADMR in metallic altermagnets coupled to ferromagnetic insulators, symmetry- and orientation-based comparison between SSMR and SMR, yielding explicit, testable transport criteria (longitudinal sign reversal, dependence only on relative Néel–magnetization orientation, and proportionality between longitudinal and transverse ADMR).  
**Tags**: `altermagnetic,angle-dependent_magnetoresistance,spin-splitter_magnetoresistance,spin-hall_magnetoresistance,ferromagnetic_insulator,transport,spintronics,collinear_altermagnetism`

<details>
  <summary>Abstract</summary>
We develop a theory of angle-dependent magnetoresistance (ADMR) in metallic altermagnets coupled to ferromagnetic insulators and establish criteria that distinguish them from conventional compensated magnets with spin-orbit coupling. We show that the spin-splitter magnetoresistance (SSMR) reported by H. Chen et al. [Adv. Mater. 37, 2507764 (2025)] constitutes a smoking-gun signature of collinear altermagnetism in metallic systems. In contrast to spin-Hall magnetoresistance (SMR), SSMR exhibits three key distinctions: it depends solely on the relative orientation between the ferromagnetic magnetization and the altermagnetic Néel vector, yields a longitudinal ADMR response of opposite sign, and features a direct proportionality between longitudinal and transverse ADMR signals, absent in SMR. These results provide a clear route to unambiguously identify altermagnets in transport.
</details>


### [13] [Effect of spin-dependent tunneling and intervalley scattering in magnetic-semiconductor van der Waals heterostructures on exciton and trion polarization](https://arxiv.org/abs/2605.09088)
*V. N. Mantsevich*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09088)

**TL;DR**: The paper develops a self-consistent theoretical model for valley pseudospin control in TMD monolayers proximitized by a 2D magnetic layer, explaining photoluminescence (PL) polarization features in TMD/magnetic van der Waals heterostructures via the interplay of spin-dependent interlayer charge transfer and intervalley scattering of excitons and trions. It predicts PL-polarization sign switching controlled by the relative timescales of electron tunneling, intervalley scattering and radiative decay, and generalizes to include intervalley/intravalley scattering between bright and dark excitons.  
**Method**: Self-consistent dynamical (rate-equation/kinetic) modelling of exciton and trion populations including spin-dependent interlayer tunneling (magnetic proximity), intervalley and intravalley scattering channels, and radiative lifetimes; analysis of resulting PL polarization dynamics under circularly polarized excitation.  
**Tags**: `valley_pseudospin, transition_metal_dichalcogenide, magnetic_proximity, van_der_Waals_heterostructure, excitons, trions, photoluminescence, intervalley_scattering, spin-dependent_tunneling`

<details>
  <summary>Abstract</summary>
We present a theoretical analysis of valley pseudospin control in the transition metal dichalcogenide (TMD) monolayer by utilizing the magnetic proximity effect of 2D magnetic layer and, propose self-consistent analysis of photoluminescence (PL) polarization peculiarities in TMD/magnetic material van der Waals heterostructures. We attribute observed peculiarities to the interplay between spin-dependent interlayer charge transfer and intervalley scattering of excitons and trions. The ratio between the electron tunneling timescale and the exciton and trion intervalley scattering lifetimes and radiative lifetimes determine the PL dynamics. A possibility to switch PL polarization sign due to the quasi-particles dynamics under circularly polarized laser excitations is revealed. We also discuss generalization of the proposed model due to the careful analysis of both intervalley and intravalley scattering processes between bright and dark excitons. Obtained results allow a long-distance manipulation of exciton and trion behaviors and open the possibilities for the effective control under spin and valley pseudospin in multilayer magnetic-semiconductor van der Waals heterostructures.
</details>


### [14] [Localization phase diagram of the Hexagonal Lattice with irrational magnetic flux](https://arxiv.org/abs/2605.09974)
*Qi Gao, Shuo Zhang, Wei Chen*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.09974)

**TL;DR**: The authors solve the Hofstadter model on a hexagonal lattice with irrational magnetic flux by reducing it to a 2×2 transfer-matrix problem and applying Avila’s global theory, obtaining an exact localization phase diagram with three pure phases (extended, localized, critical) and no mobility edge (protected by chiral symmetry). Results are cross-checked by a renormalization-group analysis for part of the diagram and by numerical computation of fractal dimensions for the remainder, with agreement between methods.  
**Method**: Reduction to a 2×2 transfer matrix for nearest-neighbor hexagonal Hofstadter model; exact solution via Avila’s global theory for irrational flux; complementary renormalization-group analysis and numerical calculation of fractal dimensions to verify the phase diagram.  
**Tags**: `Hofstadter_model, quasiperiodic_localization, transfer_matrix, Avila_global_theory, renormalization_group, chiral_symmetry, fractal_dimension`

<details>
  <summary>Abstract</summary>
We study the Hofstadter model on a hexagonal lattice with irrational magnetic flux in this work. The Hofstadter model of the square lattice with irrational flux has been solved mathematically by Avila in his Fields medal work. However, this theory is usually not applicable to lattices with internal degrees of freedom, such as spin or sub-lattices. In this work, we show that for the hexagonal lattice with only nearest neighbor hopping, the system can still be characterized by a 2*2 transfer matrix and solved exactly by Avila$'$s global theory of Avila although this lattice has two sub-lattices. We obtained the exact localization phase diagram of the hexagonal lattice with irrational flux by this theory, which reveals three pure phases, that is, the extended, localized and critical states but no mobility edge due to the chiral symmetry. We used the renormalization group (RG) theory to verify these results, which can determine part of the phase diagram. We then computed the fractal dimension of the remaining part numerically. The results from both the RG theory and numerical analysis confirmed the phase diagram we get from Avila$'$s global theory.
</details>


### [15] [One-dimensional relativistic hydrogen-like atom in Dirac materials: Energy spectra and supercritical states](https://arxiv.org/abs/2605.10208)
*S. Z. Rakhmanov, K. P. Matchonov, A. K. Rakhimov, D. U. Matrasulov*

**Category**: cond-mat.mes-hall  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10208)

**TL;DR**: Model of a 1D relativistic hydrogen-like atom realized by a Coulomb impurity in a graphene nanoribbon. The authors solve the one-dimensional Dirac equation with a finite-size Coulomb potential to obtain eigenvalues and eigenfunctions for both unconfined (free) and confined (atom-in-box) geometries. They identify supercritical energy levels, compute the critical charge (charge at which a bound state reaches the Dirac-sea threshold), and find that confinement increases the critical charge. Local density of states (LDOS) is computed and plotted, showing strong localization for the confined atom-in-box.  
**Method**: Analytical/numerical solution of the one-dimensional Dirac equation with a finite-size Coulomb potential for graphene nanoribbon geometry; calculation of eigenvalues and eigenfunctions for free and confined boundary conditions; analysis of supercritical level formation and determination of critical charge; computation/plotting of local density of states to characterize localization.  
**Tags**: `graphene, Dirac_equation, impurity_physics, Coulomb_impurity, supercriticality, critical_charge, confinement, local_density_of_states, bound_states`

<details>
  <summary>Abstract</summary>
We consider a model of 1D relativistic hydrogen-like atom, formed by a Coulomb impurity in graphene nanoribbon. Describing the electron motion in terms of the one-dimensional Dirac equation for Coulomb potential taking into account the finite-size of the atomic nucleus, we compute the eigenvalues and eigenfunctions of the atomic electron. The cases of unconfined atom and atomin-box system are considered. Special focus is given calculation of supercritical energy levels and the critical charge. The latter is the value of the atomic nucleus charge, when the electronic state reaches the border of the Dirac sea. It is found that for confined atom the value of the critical charge is larger than that of free atom. Experimentally measurable characteristics, local density of states is also plotted for both cases. Existence of strong localization for atom-in-box system is shown.
</details>


<div id='cond-mat.supr-con'></div>

# cond-mat.supr-con [[Back]](#toc)

### [16] [Transverse Magnetic Response from Orbitally Polarized Cooper Pairs in Elemental Superconductors](https://arxiv.org/abs/2605.10700)
*Gabor Csire, Maria Teresa Mercaldo, Balazs Ujfalussy, Carmine Ortix, Mario Cuoco*

**Category**: cond-mat.supr-con  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10700)

**TL;DR**: Lowering crystalline symmetry (e.g., via strain) in elemental superconductors (V, Nb) converts trigonal symmetry to C_s and activates interorbital, spin-singlet Cooper pairs composed of electrons with identical orbital moments (orbitally polarized pairs), with pronounced enhancement at (111) surfaces. In an in-plane magnetic field that breaks the remaining mirror symmetry this state produces a transverse orbital magnetization perpendicular to the field, offering an experimentally accessible signature and positioning strained elemental superconductors as a minimal platform for superconducting orbitronics.  
**Method**: First-principles superconducting density functional theory (SCDFT) calculations of bulk and (111) surface electronic structure and pairing under symmetry lowering (trigonal→C_s) and analysis of the magnetic (orbital) response to in-plane fields that break mirror symmetry.  
**Tags**: `superconductivity, orbital_pairing, strain_engineering, superconducting_density_functional_theory, orbitronics, surface_superconductivity, orbital_magnetization, vanadium, niobium`

<details>
  <summary>Abstract</summary>
We demonstrate how crystalline symmetry lowering, as for instance through strain, allows elemental superconductors such as vanadium and niobium to realize spin-singlet orbitally polarized Cooper pairs composed of electrons with identical orbital moments. Using superconducting density functional theory, we show that lowering of trigonal symmetry to $C_s$, thus keeping only a single mirror plane, activates interorbital pairing in bulk and (111) surfaces, with a pronounced surface enhancement. In a magnetic field, the resulting orbitally polarized superconducting state leads to a novel transverse magnetic response. For in--plane field orientations that break the remaining mirror symmetry, a sizable orbital magnetization emerges perpendicular to the applied field. We show that this effect is a direct consequence of equal--orbital-moment Cooper pairing, providing an experimentally accessible signature of this state. Our results establish strained elemental superconductors as a minimal material platform for superconducting orbitronics.
</details>


### [17] [Anomalous and diode Josephson effect in junctions with inhomogeneous ferromagnetic barrier and interfacial Rashba spin-orbit coupling](https://arxiv.org/abs/2605.10740)
*Stevan Djurdjević, Zorica Popović*

**Category**: cond-mat.supr-con  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10740)

**TL;DR**: Identifies minimal symmetry-breaking requirements for anomalous (φ0) and diode Josephson effects in planar 2D S–F–F–S junctions with interfacial Rashba SOC and s- or d-wave electrodes, classifies junction types (including d_{x^2-y^2}–d_{xy}), and demonstrates via numerical CPR and Andreev bound-state analysis that tuning exchange-field directions, SOC and order-parameter orientation can enhance nonreciprocity (>40%). Highlights important continuum-state contributions when ABS cross zero or d-wave gaps close.  
**Method**: Systematic symmetry analysis of the junction Hamiltonian and classification of junction classes, supplemented by numerical calculation of current–phase relations using a generalized Furusaki–Tsukada scattering approach and phase-resolved Andreev bound-state (ABS) spectrum analysis (comparison of ABS-carried current vs full F–T results).  
**Tags**: `superconductivity, Josephson_junction, anomalous_Josephson_effect, Josephson_diode, spin-orbit_coupling, ferromagnetism, d-wave_superconductivity, Andreev_bound_states, Furusaki-Tsukada, symmetry_analysis`

<details>
  <summary>Abstract</summary>
We theoretically investigate the anomalous and diode Josephson effects in planar two-dimensional Josephson junctions with arbitrarily oriented exchange fields in two ferromagnets within the barrier, and spin-orbit coupling at the superconductor/ferromagnet interfaces, where the superconducting electrodes can have $s$-wave or arbitrarily oriented $d$-wave order parameter lobes. We perform a systematic symmetry analysis of the junction Hamiltonian and identify the minimal conditions for breaking time-reversal and space-inversion symmetries, which are required for the emergence of anomalous and diode Josephson effects. We classify the junctions into three classes, with particular attention to those between $d_{x^2-y^2}$ and $d_{xy}$ oriented superconductors. Our symmetry analysis is supported by numerical calculations of the current-phase relation (CPR) obtained using a generalized Furusaki-Tsukada (F-T) approach. By tuning the directions of exchange fields in the ferromagnets, Rashba SOC at the interfaces and superconducting order parameter orientations, nonreciprocity can be enhanced by more than 40\%. We further analyze the phase-dependent Andreev bound states (ABS) spectrum and their contribution to charge transport, as well as their signatures in the nonreciprocal transport characteristics. By comparing the current carried by ABS with that obtained using the F-T technique, we find that the contribution from continuum states above the gap becomes pronounced in presence of zero energy crossings in the ABS spectrum, and in junctions with $d$-wave superconducting electrodes due to the narrower superconducting gap, which may become closed. In the nonreciprocal regime, the ABS spectra show an asymmetric profile with respect to phase inversion, indicating the presence of a finite current at zero phase difference and unequal critical currents in opposite directions.
</details>


### [18] [Ginzburg--Landau Theory for Confined Thin-Film Superconductors](https://arxiv.org/abs/2605.10686)
*Giovanni A. Ummarino, Alessio Zaccone*

**Category**: cond-mat.supr-con  
**Abstract**: [arXiv](https://arxiv.org/abs/2605.10686)

**TL;DR**: Quantum confinement in metallic thin films renormalizes intrinsic superconducting length scales: it reduces the coherence length (via modified DOS and Fermi energy) and increases the penetration depth, driving thinner films toward stronger type-II behaviour. The theory predicts a crossover where confinement effects and transport scattering (surface/disorder) interplay and explains measured penetration-depth enhancement in Al films.  
**Method**: Analytical derivation of a Ginzburg–Landau theory for confined superconducting films starting from the microscopic BCS free energy combined with a quantum confinement theory for metallic thin films; yields explicit analytical expressions for GL coefficients, coherence length, penetration depth, electronic mean free path and GL parameter, and comparison with experimental penetration-depth data.  
**Tags**: `superconductivity, quantum_confinement, Ginzburg-Landau, BCS, coherence_length, penetration_depth, thin_films, transport_scattering, type-II`

<details>
  <summary>Abstract</summary>
We develop a Ginzburg--Landau theory for superconducting thin films under quantum confinement. Starting from the microscopic BCS free energy and the recently developed confinement theory of metallic thin films, explicit analytical expressions are derived for the Ginzburg--Landau coefficients, coherence length, penetration depth, electronic mean free path, and Ginzburg--Landau parameter in confined geometries. The central result is that quantum confinement directly renormalizes the intrinsic superconducting coherence length through confinement-induced modifications of the electronic density of states and Fermi energy. This effect is absent in conventional thin-film transport theories based solely on surface scattering. As a consequence, confinement simultaneously suppresses the coherence length and enhances the penetration depth, thereby driving superconductors toward progressively stronger type-II behavior with decreasing film thickness. The theory predicts a crossover regime in which confinement-induced renormalization of superconducting length scales and transport scattering become strongly intertwined. Comparison with recent penetration-depth measurements in Al thin films shows that the observed enhancement of the penetration depth originates from the interplay between confinement-induced renormalization of the coherence length and suppression of the effective mean free path by surface and disorder scattering. The results establish a direct connection between quantum confinement and superconducting electrodynamics in confined metallic films.
</details>
